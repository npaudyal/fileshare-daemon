...
impl PeerManager {
    ...
    pub async fn handle_message_without_clipboard(
        &mut self,
        peer_id: Uuid,
        message: Message,
    ) -> Result<()> {
        ...
        match message.message_type {
            ...
            // Skip clipboard messages since they're handled in main.rs now
            MessageType::ClipboardUpdate { .. } => {
                info!(
                    "Received clipboard update from {} - handled by main thread",
                    peer_id
                );
            }
            ...
            MessageType::ClipboardClear => {
                info!(
                    "Received clipboard clear from {} - handled by main thread",
                    peer_id
                );
            }
